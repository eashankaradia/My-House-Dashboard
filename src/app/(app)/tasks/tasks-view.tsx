...
import { createTask, deleteTask, toggleTask } from "@/app/(app)/projects/actions";
        ...
        <CardContent className="space-y-1.5">
          {outstanding.length === 0 ? (
            ...
            outstanding.map((task) => (
              <TaskRow key={task.id} task={task} project={projectName(task.project_id)} memberMap={memberMap} />
            ))
          )}
        </CardContent>
      </Card>
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border p-2.5">
      ...
      <div className="min-w-0 flex-1">
        <span className={cn("text-sm", task.is_done && "text-muted-foreground line-through")}>{task.title}</span>
