...
import { INSPIRATION_KINDS, INSPIRATION_KIND_LABELS } from "@/lib/constants";
import type { NutritionInspiration } from "@/lib/database.types";
import { createNutritionInspiration, updateNutritionInspiration, deleteNutritionInspiration } from "./actions";

type Props = { item?: NutritionInspiration; trigger?: React.ReactNode };

export function NutritionInspirationForm({ item, trigger }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      const result = editing ? await updateNutritionInspiration(item!.id, payload) : await createNutritionInspiration(payload);
      if (result?.error) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{editing ? "Edit" : "Save nutrition inspiration"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4">
          <Field label="Type">
            <NativeSelect value={kind} onChange={(e) => setKind(e.target.value)}>
              {INSPIRATION_KINDS.map((k) => (
                <option key={k} value={k}>
                  {INSPIRATION_KIND_LABELS[k]}
                </option>
              ))}
            </NativeSelect>
          </Field>
          <Field label="Title" required>
            <Input placeholder="e.g. High-protein meal prep ideas" value={title} onChange={(e) => setTitle(e.target.value)} required autoFocus />
          </Field>
          {kind === "reel" ? (
          ...
          <DialogFooter className={editing ? "sm:justify-between" : undefined}>
            {editing && (
              <FormDeleteButton
                label="Delete"
                onDelete={async () => {
                  const r = await deleteNutritionInspiration(item!.id);
                  if (!r?.error) {
                    toast({ title: "Deleted" });
                    setOpen(false);
                  }
