...
import { createFinanceInspiration, updateFinanceInspiration, deleteFinanceInspiration } from "./actions";

type Props = { item?: FinanceInspiration; trigger?: React.ReactNode };

export function FinanceInspirationForm({ item, trigger }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      const result = editing ? await updateFinanceInspiration(item!.id, payload) : await createFinanceInspiration(payload);
      if (result?.error) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{editing ? "Edit" : "Save finance inspiration"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <Field label="Title" required>
            <Input placeholder="e.g. How to build an emergency fund" value={title} onChange={(e) => setTitle(e.target.value)} required autoFocus />
          </Field>
          {kind === "reel" ? (
          ...
          <DialogFooter className={editing ? "sm:justify-between" : undefined}>
            {editing && (
              <FormDeleteButton
                label="Delete"
                onDelete={async () => {
                  const r = await deleteFinanceInspiration(item!.id);
                  if (!r?.error) {
                    toast({ title: "Deleted" });
                    setOpen(false);
                  }
